...
def _parse_generic(rows, headers) -> list[dict]:
    """
    Tries a best-effort generic approach:
    Looks for columns named date, description/narration/particulars, debit/withdrawal, credit/deposit
    """
    parsed = []
    h = [x.strip().lower() for x in headers]
...
async def import_csv(
    ...
    bank_format = _detect_format(headers)

    if bank_format == "HDFC":
        parsed = _parse_hdfc(data_rows, headers)
    elif bank_format == "SBI":
        parsed = _parse_sbi(data_rows, headers)
    else:
        parsed = _parse_generic(data_rows, headers)
